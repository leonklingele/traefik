...
|  [8] | `clientAuth.crlFiles`              | defines local CRL files or CRL content used to do client certificate revocation checks |
|  [9] | `clientAuth.revocationCheckStrict` | if `true`, Traefik won't allow connections from clients connections without successfully performing required revocation checks                                                            |
| [10] | `sniStrict`                        | if `true`, Traefik won't allow connections from clients connections that do not specify a server_name extension                                                            |
